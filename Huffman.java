...
will go on the right. Repeat until the tree is complete, then fill in the 's and 's for the edges. The finished graph looks like:
*/
import java.util.*;
...
*/ 
	...
	void decode(String s, Node root) {
	 ...
	 //if character is equal to '1' i should move to right else move to left side of tree,then i should check for children 
	 //of that node, if have children to that node ,then based on next character the string i should move left or right 
	 //('0' means left , '1' means right) if it reaches leaf i should print that node data and pointer must go back to root..
	 //process will repeat until all leafs visited	
            if(s.charAt(i)=='1'){
                temp=temp.right;
                if(i+1<s.length()){
                    if(s.charAt(i+1)=='1'){
